...
bool RX8130_Class::begin(i2c_master_bus_handle_t busHandle, uint8_t addr) {
  ...
  ESP_ERROR_CHECK(
      i2c_master_bus_add_device(busHandle, &dev_cfg, &_i2c_device_handle));
...
void RX8130_Class::setTime(struct tm *time) {
  ...
  uint8_t date[7] = {dec2bcd(time->tm_sec),       dec2bcd(time->tm_min),
                     dec2bcd(time->tm_hour),      dec2bcd(time->tm_wday),
                     dec2bcd(time->tm_mday),      dec2bcd(time->tm_mon),
...
void RX8130_Class::getTime(struct tm *time) {
  ...
  time->tm_hour =
      bcd2dec(date[RX8130_REG_HOUR - 0x10] & 0x3f); // only 24-hour clock
...
void RX8130_Class::clearIrqFlags() { writeRegister8(RX8130_REG_FLAG, 0); }

void RX8130_Class::disableIrq() { writeRegister8(RX8130_REG_CTRL0, 0); }
...
void RX8130_Class::readRegister(uint8_t reg, uint8_t *buf, uint8_t len) {
  ...
  w_buffer[0] = reg;
  i2c_master_transmit_receive(_i2c_device_handle, w_buffer, 1, buf, len,
                              portMAX_DELAY);
}
