...
void hid_host_device_callback(hid_host_device_handle_t hid_device_handle,
  ...
  const app_event_queue_t evt_queue = {
      ...
      .hid_host_device = {
          .handle = hid_device_handle, .event = event, .arg = arg}};
...
static void tab5_usb_host_task(void *pvParameters) {
  ...
  // xTaskGetCurrentTaskHandle(), 2, NULL, 0); assert(task_created == pdTRUE);
...
static void lvgl_mouse_read_cb(lv_indev_t *indev, lv_indev_data_t *data) {
  ...
  data->state = GetHAL()->hidMouseData.btnLeft ? LV_INDEV_STATE_PRESSED
                                               : LV_INDEV_STATE_RELEASED;
}

void HalEsp32::hid_init() {
  mclog::tagInfo(TAG, "hid init");
  xTaskCreatePinnedToCore(tab5_usb_host_task, "usba", 4096 * 2, NULL, 5, NULL,
                          0);
