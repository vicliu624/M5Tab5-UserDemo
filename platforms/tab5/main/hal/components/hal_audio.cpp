...
uint8_t HalEsp32::getSpeakerVolume() { return _current_speaker_volume; }

void HalEsp32::audioRecord(std::vector<int16_t> &data, uint16_t durationMs,
                           float gain) {
  ...
  size_t bytes_read = 0;
  codec_handle->i2s_read((char *)data.data(),
                         (48000 * 4 * durationMs / 1000) * sizeof(uint16_t),
  ...
  while (true) {
    ...
    if (_audio_task_data.is_audio_ready) {
      ...
      codec_handle->i2s_write(_audio_task_data.audio_data.data(),
                              _audio_task_data.audio_data.size() *
                                  sizeof(uint16_t),
...
void HalEsp32::audioPlay(std::vector<int16_t> &data, bool async) {
  if (async) {
    ...
    size_t bytes_written = 0;
    codec_handle->i2s_reconfig_clk_fn(48000, 16, I2S_SLOT_MODE_STEREO);
    codec_handle->i2s_write(data.data(), data.size() * sizeof(uint16_t),
                            &bytes_written, portMAX_DELAY);
  }
}
...
static void _rec_test_task(void *param) {
  ...
  while (total_read_samples < total_samples) {
    ...
    size_t bytes_read = 0;
    codec_handle->i2s_read((char *)(read_buf + total_read_samples),
  ...
  codec_handle->i2s_write(_rec_test_data.audio_buffer,
                          (48000 * 2 * 3) * sizeof(uint16_t), &bytes_written,
                          portMAX_DELAY);
...
void HalEsp32::startDualMicRecordTest() { try_create_rec_test_task(true); }
...
extern const uint8_t
    startup_sfx_mp3_start[] asm("_binary_startup_sfx_mp3_start");
extern const uint8_t startup_sfx_mp3_end[] asm("_binary_startup_sfx_mp3_end");
extern const uint8_t
    shutdown_sfx_mp3_start[] asm("_binary_shutdown_sfx_mp3_start");
