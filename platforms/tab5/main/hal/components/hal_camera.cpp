...
/**
 * @brief   Open the video device and initialize the video device to use
 * `init_fmt` as the output format.
 * @note    When the sensor outputs data in RAW format, the ISP module can
 * interpolate its data into RGB or YUV format. However, when the sensor works
 * in RGB or YUV format, the output data can only be in RGB or YUV format.
...
void app_camera_display(void *arg) {
  ...
  if (img_show == NULL) {
    ...
    img_show_data = (uint8_t *)heap_caps_calloc(
        img_show_size, 1, MALLOC_CAP_DMA | MALLOC_CAP_SPIRAM);
  ...
  while (1) {
    ...
    bsp_display_lock(0);
    lv_canvas_set_buffer(camera_canvas, img_show->data, CAMERA_WIDTH,
                         CAMERA_HEIGHT, LV_COLOR_FORMAT_RGB565);
    bsp_display_unlock();
    ...
    if (xQueueReceive(queue_camera_ctrl, &task_control, 0) == pdPASS) {
      if (task_control == TASK_CONTROL_PAUSE) {
        ...
        if (xQueueReceive(queue_camera_ctrl, &task_control, portMAX_DELAY) ==
            pdPASS) {
...
void HalEsp32::startCameraCapture(lv_obj_t *imgCanvas) {
  ...
  xTaskCreatePinnedToCore(app_camera_display, "cam", 8 * 1024, NULL, 5, NULL,
                          1);
}
