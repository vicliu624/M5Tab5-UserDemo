...
#define ISP_AE_REGIONS                                                         \
  (ISP_AE_BLOCK_X_NUM * ISP_AE_BLOCK_Y_NUM) /*!< Auto exposure regions */

/**
 * @brief ISP statistics flags.
 */
#define IPA_STATS_FLAGS_AWB                                                    \
  (1 << 0) /*!< ISP statistics has auto white balance */
...
typedef struct esp_ipa_sensor {
  ...
  uint32_t step_exposure; /*!< Step exposure, unit is micro second us; if
                             step_exposure == 0, step size is uneven */
...
typedef struct esp_ipa_denoising_bf {
  ...
  uint8_t matrix[ISP_BF_TEMPLATE_X_NUMS]
                [ISP_BF_TEMPLATE_Y_NUMS]; /*!< BF filter matrix */
...
typedef struct esp_ipa_sharpen {
  ...
  float m_coeff;    /*!< Sharpen coefficient of middle threshold(value between
                       "l_thresh" and "h_thresh" ) */
  uint8_t matrix[ISP_SHARPEN_TEMPLATE_X_NUMS]
                [ISP_SHARPEN_TEMPLATE_Y_NUMS]; /*!< Sharpen filter matrix */
} esp_ipa_sharpen_t;
...
typedef struct esp_ipa_ccm {
  float matrix[ISP_CCM_DIMENSION]
              [ISP_CCM_DIMENSION]; /*!< Color correction matrix */
} esp_ipa_ccm_t;
...
/**
 * @brief IPA meta data, these data are calculated by IPA and configured to ISP
 * hardware
 */
...
typedef struct esp_ipa_ops {
  /**
   * @brief Initialize IPA, this function generally contains the following
   * steps:
   *
   ...
   */
  esp_err_t (*init)(struct esp_ipa *ipa, const esp_ipa_sensor_t *sensor,
                    esp_ipa_metadata_t *metadata);

  /**
   * @brief Run IPA calculation function once to generate metadata, the metadata
   * will be written into ISP or sensor. If the IPA just only has an
   * initialization function and doesn't need a dynamical calculation function,
   * the "process" can be set to be NULL.
   */
  void (*process)(struct esp_ipa *ipa, const esp_ipa_stats_t *stats,
