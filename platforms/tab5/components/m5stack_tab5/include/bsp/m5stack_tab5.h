...
/* Audio */
#define BSP_I2S_SCLK                                                           \
  (GPIO_NUM_27) // 位时钟         BSP_I2S_BCLK  <--> ES7210/ESP311 I2S_BCLK
#define BSP_I2S_MCLK                                                           \
  (GPIO_NUM_30) // 主时钟         BSP_I2S_MCLK  <--> ES7210/ESP311 I2S_MCLK
#define BSP_I2S_LCLK                                                           \
  (GPIO_NUM_29) // 字(声道)选择   BSP_I2S_WR    <--> ES7210/ESP311 I2S_WR
#define BSP_I2S_DOUT                                                           \
  (GPIO_NUM_26) // 数据输出       BSP_I2S_DOUT  ---> ES8388        I2S_DSIN
#define BSP_I2S_DSIN                                                           \
  (GPIO_NUM_28) // 数据输入       BSP_I2S_DIN   <--- ES7210        I2S_DOUT
 ...
 *
 * For speaker initialization use bsp_audio_codec_speaker_init() which is inside
 * initialize I2S with bsp_audio_init(). For microphone initialization use
 * bsp_audio_codec_microphone_init() which is inside initialize I2S with
 *bsp_audio_init(). After speaker or microphone initialization, use functions
 * from esp_codec_dev for play/record audio. Example audio play:
 * \code{.c}
...
typedef esp_err_t (*bsp_i2s_read_fn)(void *audio_buffer, size_t len,
                                     size_t *bytes_read, uint32_t timeout_ms);
typedef esp_err_t (*bsp_i2s_write_fn)(void *audio_buffer, size_t len,
...
typedef esp_err_t (*bsp_codec_reconfig_fn)(uint32_t rate, uint32_t bps,
                                           i2s_slot_mode_t ch);
typedef esp_err_t (*bsp_i2s_reconfig_clk_fn)(uint32_t rate, uint32_t bits_cfg,
                                             i2s_slot_mode_t ch);
 ...
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_NOT_FOUND if the partition table does not contain SPIFFS
 ...
 *    - ESP_ERR_NO_MEM          If not enough memory or too many VFSes already
 * registered
 *    - Others                  Fail
 */
 ...
 * It features 16-bit colors, 320x240 resolution and capacitive touch
 * controller.
 *
 * LVGL is used as graphics library. LVGL is NOT thread safe, therefore the user
 * must take LVGL mutex by calling bsp_display_lock() before calling and LVGL
 * API (lv_...) and then give the mutex with bsp_display_unlock().
 *
 * Display's backlight must be enabled explicitly by calling
...
#define BSP_LCD_DRAW_BUFF_SIZE                                                 \
  (BSP_LCD_H_RES * 50) // Frame buffer size in pixels
...
typedef struct {
  ...
  struct {
    ...
    unsigned int sw_rotate : 1; /*!< Use software rotation (slower), The feature
                                   is unavailable under avoid-tear mode */
  } flags;
} bsp_display_cfg_t;
 ...
 *
 * This function initializes SPI, display controller and starts LVGL handling
 * task. LCD backlight must be enabled separately by calling
 ...
 *
 * This is a one-stop-shop function that will configure the board for USB Host
 * mode and start USB Host library
 *
