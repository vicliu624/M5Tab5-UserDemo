 ...
 *
 * @note  This structure needs to be passed to the
 * `esp_lcd_panel_dev_config_t::vendor_config`.
 *
 */
typedef struct {
  const st7123_lcd_init_cmd_t
  ...
  struct {
    ...
    const esp_lcd_dpi_panel_config_t
        *dpi_config;  /*!< MIPI-DPI panel configuration */
 ...
 */
esp_err_t
esp_lcd_new_panel_st7123(const esp_lcd_panel_io_handle_t io,
 ...
 *
 * @note  refresh_rate = (dpi_clock_freq_mhz * 1000000) / (h_res +
 * hsync_pulse_width + hsync_back_porch + hsync_front_porch) / (v_res +
