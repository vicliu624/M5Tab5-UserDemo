...
typedef struct {
  ...
  unsigned int
      min_unalign_byte; // Minimum amount of unaligned bytes of the test array
  unsigned int
      max_unalign_byte; // Maximum amount of unaligned bytes of the test array
  unsigned int
      unalign_step; // Increment step in bytes unalignment of the test array
  unsigned int dest_stride_step; // Increment step in destination stride of the
                                 // test array
...
typedef struct {
  struct {
    ...
    void *p_asm_alloc;  // pointer to the beginning of the memory allocated for
                        // ASM test buf, used in free()
    void *p_ansi_alloc; // pointer to the beginning of the memory allocated for
                        // ANSI test buf, used in free()
  } buf;
  void (*blend_api_func)(
      _lv_draw_sw_blend_fill_dsc_t *); // pointer to LVGL API function
  void (*blend_api_px_func)(
  ...
  size_t active_buf_len; // Length of buffer, where the actual data are stored
                         // (not including Canary bytes)
...
typedef struct {
  ...
  void *array_align16; // test array with 16 byte alignment - testing most ideal
                       // case
