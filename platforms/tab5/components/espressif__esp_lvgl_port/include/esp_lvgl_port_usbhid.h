 ...
 *
 * @note The USB host must be initialized before. Use `usb_host_install` for
 * host initialization.
 *
 ...
 */
lv_indev_t *
lvgl_port_add_usb_hid_mouse_input(const lvgl_port_hid_mouse_cfg_t *mouse_cfg);
...
lv_indev_t *lvgl_port_add_usb_hid_keyboard_input(
    const lvgl_port_hid_keyboard_cfg_t *keyboard_cfg);

/**
 * @brief Remove selected USB HID from input devices
 *
 * @note Free all memory used for this input device. When removed all HID
 * devices, the HID task will be freed.
 *
 * @return
 *      - ESP_OK                    on success
 */
