...
typedef struct {
  ...
  lvgl_port_rotation_cfg_t
      rotation; /*!< Default values of the screen rotation (Only HW state. Not
                   supported for default SW rotation!) */
  ...
  struct {
    ...
    unsigned int sw_rotate
        : 1; /*!< Use software rotation (slower) or PPA if available */
#if LVGL_VERSION_MAJOR >= 9
    unsigned int swap_bytes : 1; /*!< Swap bytes in RGB656 (16-bit) color format
                                    before send to LCD driver */
#endif
    unsigned int full_refresh
        : 1; /*!< 1: Always make the whole screen redrawn */
    unsigned int direct_mode : 1; /*!< 1: Use screen-sized buffers and draw to
...
typedef struct {
  struct {
    ...
    unsigned int avoid_tearing
        : 1; /*!< 1: Use internal RGB buffers as a LVGL draw buffers to avoid
                tearing effect, enabling this option requires over two LCD
                buffers and may reduce the frame rate */
  } flags;
...
typedef struct {
  struct {
    unsigned int avoid_tearing
        : 1; /*!< 1: Use internal MIPI-DSI buffers as a LVGL draw buffers to
                avoid tearing effect, enabling this option requires over two LCD
                buffers and may reduce the frame rate */
  } flags;
 ...
 * lvgl_port_remove_disp for free all memory!
 *
 ...
 */
lv_display_t *
lvgl_port_add_disp_dsi(const lvgl_port_display_cfg_t *disp_cfg,
 ...
 */
lv_display_t *
lvgl_port_add_disp_rgb(const lvgl_port_display_cfg_t *disp_cfg,
