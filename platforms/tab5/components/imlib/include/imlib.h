...
#define IM_LOG2_4(x)                                                           \
  (((x) & 0xCULL) ? (2 + IM_LOG2_2((x) >> 2)) : IM_LOG2_2(x)) // NO ({ ... }) !
#define IM_LOG2_8(x)                                                           \
  (((x) & 0xF0ULL) ? (4 + IM_LOG2_4((x) >> 4)) : IM_LOG2_4(x)) // NO ({ ... }) !
#define IM_LOG2_16(x)                                                          \
  (((x) & 0xFF00ULL) ? (8 + IM_LOG2_8((x) >> 8))                               \
                     : IM_LOG2_8(x)) // NO ({ ... }) !
#define IM_LOG2_32(x)                                                          \
  (((x) & 0xFFFF0000ULL) ? (16 + IM_LOG2_16((x) >> 16))                        \
                         : IM_LOG2_16(x)) // NO ({ ... }) !
#define IM_LOG2(x)                                                             \
  (((x) & 0xFFFFFFFF00000000ULL) ? (32 + IM_LOG2_32((x) >> 32))                \
                                 : IM_LOG2_32(x)) // NO ({ ... }) !

#define IM_IS_SIGNED(a)                                                        \
  (__builtin_types_compatible_p(__typeof__(a), signed) ||                      \
...
#define IM_SIGN_COMPARE(a, b)                                                  \
  ((IM_IS_SIGNED(a) && IM_IS_UNSIGNED(b)) ||                                   \
...
#define IM_MAX(a, b)                                                           \
  ({                                                                           \
    __typeof__(a) _a = (a);                                                    \
    __typeof__(b) _b = (b);                                                    \
    __builtin_choose_expr(IM_SIGN_COMPARE(_a, _b), (void)0,                    \
                          (_a > _b ? _a : _b));                                \
  })
...
void point_rotate(int x, int y, float r, int center_x, int center_y,
...
#define COLOR_BOUND_RGB565(pixel0, pixel1, threshold)                          \
  ({                                                                           \
    ...
    __typeof__(pixel1) _pixel1 = (pixel1);                                     \
    __typeof__(threshold) _threshold = (threshold);                            \
    (abs(COLOR_RGB565_TO_R5(_pixel0) - COLOR_RGB565_TO_R5(_pixel1)) <=         \
     COLOR_RGB565_TO_R5(_threshold)) &&                                        \
        (abs(COLOR_RGB565_TO_G6(_pixel0) - COLOR_RGB565_TO_G6(_pixel1)) <=     \
         COLOR_RGB565_TO_G6(_threshold)) &&                                    \
        (abs(COLOR_RGB565_TO_B5(_pixel0) - COLOR_RGB565_TO_B5(_pixel1)) <=     \
         COLOR_RGB565_TO_B5(_threshold));                                      \
  })
...
#define COLOR_RGB888_TO_U(r8, g8, b8)                                          \
  ((((r8) * -21) - ((g8) * 43) + ((b8) * 64)) >>                               \
   7) // -0.168736R - 0.331264G + 0.5B
...
#define COLOR_RGB888_TO_V(r8, g8, b8)                                          \
  ((((r8) * 64) - ((g8) * 54) - ((b8) * 10)) >>                                \
   7) // 0.5R - 0.418688G - 0.081312B
...
#define COLOR_BINARY_TO_RGB565(pixel)                                          \
  COLOR_YUV_TO_RGB565(((pixel) ? 127 : -128), 0, 0)
#define COLOR_RGB565_TO_BINARY(pixel)                                          \
  (COLOR_RGB565_TO_Y(pixel) > (((COLOR_Y_MAX - COLOR_Y_MIN) / 2) + COLOR_Y_MIN))
...
#define COLOR_GRAYSCALE_TO_RGB565(pixel)                                       \
  COLOR_YUV_TO_RGB565(((pixel) - 128), 0, 0)
...
// 31......29  28  27  26  25
// 24  23..........16  15...........8  7.............0 <RESERVED>  YF  MF  CF JF
// RF  <PIXFORMAT_ID>  <SUBFORMAT_ID>  <BYTES_PER_PIX> NOTE: Bit 31-30 must Not
// be used for pixformat_t to be used as mp_int_t.
typedef enum {
  PIXFORMAT_INVALID = (0x00000000U),
  PIXFORMAT_BINARY = (PIXFORMAT_FLAGS_M | (PIXFORMAT_ID_BINARY << 16) |
                      (0 << 8) | PIXFORMAT_BPP_BINARY),
  PIXFORMAT_GRAYSCALE = (PIXFORMAT_FLAGS_M | (PIXFORMAT_ID_GRAY << 16) |
                         (SUBFORMAT_ID_GRAY8 << 8) | PIXFORMAT_BPP_GRAY8),
  PIXFORMAT_RGB565 = (PIXFORMAT_FLAGS_CM | (PIXFORMAT_ID_RGB565 << 16) |
                      (0 << 8) | PIXFORMAT_BPP_RGB565),
  PIXFORMAT_ARGB8 = (PIXFORMAT_FLAGS_CM | (PIXFORMAT_ID_ARGB8 << 16) |
                     (0 << 8) | PIXFORMAT_BPP_ARGB8),
  PIXFORMAT_BAYER = (PIXFORMAT_FLAGS_CR | (PIXFORMAT_ID_BAYER << 16) |
                     (SUBFORMAT_ID_BGGR << 8) | PIXFORMAT_BPP_BAYER),
  PIXFORMAT_BAYER_BGGR = (PIXFORMAT_FLAGS_CR | (PIXFORMAT_ID_BAYER << 16) |
                          (SUBFORMAT_ID_BGGR << 8) | PIXFORMAT_BPP_BAYER),
  PIXFORMAT_BAYER_GBRG = (PIXFORMAT_FLAGS_CR | (PIXFORMAT_ID_BAYER << 16) |
                          (SUBFORMAT_ID_GBRG << 8) | PIXFORMAT_BPP_BAYER),
  PIXFORMAT_BAYER_GRBG = (PIXFORMAT_FLAGS_CR | (PIXFORMAT_ID_BAYER << 16) |
                          (SUBFORMAT_ID_GRBG << 8) | PIXFORMAT_BPP_BAYER),
  PIXFORMAT_BAYER_RGGB = (PIXFORMAT_FLAGS_CR | (PIXFORMAT_ID_BAYER << 16) |
                          (SUBFORMAT_ID_RGGB << 8) | PIXFORMAT_BPP_BAYER),
  PIXFORMAT_YUV = (PIXFORMAT_FLAGS_CY | (PIXFORMAT_ID_YUV422 << 16) |
                   (SUBFORMAT_ID_YUV422 << 8) | PIXFORMAT_BPP_YUV422),
  PIXFORMAT_YUV422 = (PIXFORMAT_FLAGS_CY | (PIXFORMAT_ID_YUV422 << 16) |
                      (SUBFORMAT_ID_YUV422 << 8) | PIXFORMAT_BPP_YUV422),
  PIXFORMAT_YVU422 = (PIXFORMAT_FLAGS_CY | (PIXFORMAT_ID_YUV422 << 16) |
                      (SUBFORMAT_ID_YVU422 << 8) | PIXFORMAT_BPP_YUV422),
  PIXFORMAT_JPEG =
      (PIXFORMAT_FLAGS_CJ | (PIXFORMAT_ID_JPEG << 16) | (0 << 8) | 0),
  PIXFORMAT_PNG =
      (PIXFORMAT_FLAGS_CJ | (PIXFORMAT_ID_PNG << 16) | (0 << 8) | 0),
...
#define IMLIB_PIXFORMAT_IS_VALID(x)                                            \
  ((x == PIXFORMAT_BINARY) || (x == PIXFORMAT_GRAYSCALE) ||                    \
   (x == PIXFORMAT_RGB565) || (x == PIXFORMAT_ARGB8) ||                        \
   (x == PIXFORMAT_BAYER_BGGR) || (x == PIXFORMAT_BAYER_GBRG) ||               \
   (x == PIXFORMAT_BAYER_GRBG) || (x == PIXFORMAT_BAYER_RGGB) ||               \
   (x == PIXFORMAT_YUV422) || (x == PIXFORMAT_YVU422) ||                       \
   (x == PIXFORMAT_JPEG) || (x == PIXFORMAT_PNG))
...
#define IMAGE_BINARY_LINE_LEN(image)                                           \
  (((image)->w + UINT32_T_MASK) >> UINT32_T_SHIFT)
#define IMAGE_BINARY_LINE_LEN_BYTES(image)                                     \
  (IMAGE_BINARY_LINE_LEN(image) * sizeof(uint32_t))

#define IMAGE_GRAYSCALE_LINE_LEN(image) ((image)->w)
#define IMAGE_GRAYSCALE_LINE_LEN_BYTES(image)                                  \
  (IMAGE_GRAYSCALE_LINE_LEN(image) * sizeof(uint8_t))

#define IMAGE_RGB565_LINE_LEN(image) ((image)->w)
#define IMAGE_RGB565_LINE_LEN_BYTES(image)                                     \
  (IMAGE_RGB565_LINE_LEN(image) * sizeof(uint16_t))
...
#define IMAGE_GET_BINARY_PIXEL(image, x, y)                                    \
  ({                                                                           \
    ...
    __typeof__(y) _y = (y);                                                    \
    (((uint32_t *)_image                                                       \
          ->data)[(((_image->w + UINT32_T_MASK) >> UINT32_T_SHIFT) * _y) +     \
                  (_x >> UINT32_T_SHIFT)] >>                                   \
     (_x & UINT32_T_MASK)) &                                                   \
        1;                                                                     \
  })
...
#define IMAGE_PUT_BINARY_PIXEL(image, x, y, v)                                 \
  ({                                                                           \
    ...
    __typeof__(y) _y = (y);                                                    \
    __typeof__(v) _v = (v);                                                    \
    size_t _i = (((_image->w + UINT32_T_MASK) >> UINT32_T_SHIFT) * _y) +       \
                (_x >> UINT32_T_SHIFT);                                        \
    size_t _j = _x & UINT32_T_MASK;                                            \
    ((uint32_t *)_image->data)[_i] =                                           \
        (((uint32_t *)_image->data)[_i] & (~(1 << _j))) | ((_v & 1) << _j);    \
  })
...
#define IMAGE_CLEAR_BINARY_PIXEL(image, x, y)                                  \
  ({                                                                           \
    ...
    ((uint32_t *)_image                                                        \
         ->data)[(((_image->w + UINT32_T_MASK) >> UINT32_T_SHIFT) * _y) +      \
                 (_x >> UINT32_T_SHIFT)] &= ~(1 << (_x & UINT32_T_MASK));      \
  })
...
#define IMAGE_COMPUTE_BINARY_PIXEL_ROW_PTR(image, y)                           \
  ({                                                                           \
    __typeof__(image) _image = (image);                                        \
    __typeof__(y) _y = (y);                                                    \
    ((uint32_t *)_image->data) +                                               \
        (((_image->w + UINT32_T_MASK) >> UINT32_T_SHIFT) * _y);                \
  })
...
void imlib_draw_line(image_t *img, int x0, int y0, int x1, int y1, int c,
                     int thickness);
void imlib_draw_arrow(image_t *img, int x0, int y0, int x1, int y1, int c,
                      int th, int size);
...
void imlib_draw_circle(image_t *img, int cx, int cy, int r, int c,
                       int thickness, bool fill);
void imlib_draw_ellipse(image_t *img, int cx, int cy, int rx, int ry,
                        int rotation, int c, int thickness, bool fill);
void imlib_draw_string(image_t *img, int x_off, int y_off, const char *str,
                       int c, float scale, int x_spacing, int y_spacing,
                       bool mono_space, int char_rotation, bool char_hmirror,
                       bool char_vflip, int string_rotation,
                       bool string_hmirror, bool string_hflip);

// void imlib_draw_char_8x16(image_t *fb, int32_t start_x, int32_t start_y,
// uint8_t ch, uint32_t color); void imlib_draw_char_16x16(image_t *fb, int32_t
// start_x, int32_t start_y, uint32_t code, uint32_t color); void
// imlib_draw_string(image_t *fb, uint16_t x, uint16_t y, const char *str,
// uint32_t color); void imlib_draw_pixel(image_t *fb, int x, int y, uint32_t
// color) ; void imlib_draw_line(image_t *fb, uint16_t x0, uint16_t y0, uint16_t
// x1, uint16_t y1, uint32_t color, uint16_t thickness); void
// imlib_draw_rectangle(image_t *fb, uint16_t rx, uint16_t ry, uint16_t rw,
// uint16_t rh, uint32_t color, uint16_t thickness, uint8_t fill); void
// imlib_draw_circle(image_t *fb, uint16_t cx, uint16_t cy, uint16_t radius,
// uint32_t color, uint16_t thickness, uint8_t fill);
