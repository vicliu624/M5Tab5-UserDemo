...
typedef struct esp_video_init_sccb_config {
  bool init_sccb; /*!< true:  SCCB I2C is not initialized and esp_video_init
                     function will initialize SCCB I2C with given parameters
                     i2c_config. false: SCCB I2C is initialized and
...
typedef struct esp_video_init_csi_config {
  esp_video_init_sccb_config_t
      sccb_config; /*!< Camera sensor SCCB configuration */

  int8_t reset_pin; /*!< Camera sensor reset pin, if hardware has no reset pin,
                       set reset_pin to be -1 */
  int8_t pwdn_pin;  /*!< Camera sensor power down pin, if hardware has no power
                       down pin, set pwdn_pin to be -1 */
...
typedef struct esp_video_init_jpeg_config {
  jpeg_encoder_handle_t
      enc_handle; /*!< JPEG encoder driver handle:
                       - NULL, JPEG video device will create JPEG encoder driver
                     handle by itself
                       - Not null, JPEG video device will use this handle
                     instead of creating JPEG encoder driver handle */
...
typedef struct esp_video_init_config {
  const esp_video_init_csi_config_t
      *csi; /*!< MIPI CSI initialization configuration */
  const esp_video_init_dvp_config_t
      *dvp; /*!< DVP initialization configuration array */
  const esp_video_init_jpeg_config_t
      *jpeg; /*!< JPEG initialization configuration */
  const esp_video_init_isp_config_t
      *isp; /*!< ISP initialization configuration, using default config if
               setting NULL */
} esp_video_init_config_t;

/**
 * @brief Initialize video hardware and software, including I2C, MIPI CSI and so
 * on.
 *
