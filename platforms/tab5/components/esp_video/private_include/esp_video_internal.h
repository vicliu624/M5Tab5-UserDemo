...
#define CAPTURE_VIDEO_DONE_BUF(v, b, n)                                        \
  esp_video_done_buffer(v, V4L2_BUF_TYPE_VIDEO_CAPTURE, b, n)
...
#define M2M_VIDEO_CAPTURE_BUF_SIZE(v)                                          \
  STREAM_BUFFER_SIZE(M2M_VIDEO_CAPTURE_STREAM(v))
#define M2M_VIDEO_OUTPUT_BUF_SIZE(v)                                           \
  STREAM_BUFFER_SIZE(M2M_VIDEO_OUTPUT_STREAM(v))

#define M2M_VIDEO_DONE_CAPTURE_BUF(v, b, n)                                    \
  esp_video_done_buffer(v, V4L2_BUF_TYPE_VIDEO_CAPTURE, b, n)
#define M2M_VIDEO_DONE_OUTPUT_BUF(v, b, n)                                     \
  esp_video_done_buffer(v, V4L2_BUF_TYPE_VIDEO_OUTPUT, b, n)
...
enum esp_video_event {
  ESP_VIDEO_BUFFER_VALID =
      0, /*!< Video buffer is freed and it can be allocated by video device */
 ...
 *      - Others if failed
 */
typedef esp_err_t (*esp_video_m2m_process_t)(struct esp_video *video,
...
struct esp_video_ops {
  /*!< Initialize video hardware and allocate software resource, and must set
   * buffer information and video format */
  ...
  esp_err_t (*enum_format)(struct esp_video *video, uint32_t type,
                           uint32_t index, uint32_t *pixel_format);

  /*!< Set video format configuration */

  esp_err_t (*set_format)(struct esp_video *video,
                          const struct v4l2_format *format);

  /*!< Notify driver event triggers */

  esp_err_t (*notify)(struct esp_video *video, enum esp_video_event event,
                      void *arg);

  /*!< Set external control value */

  esp_err_t (*set_ext_ctrl)(struct esp_video *video,
                            const struct v4l2_ext_controls *ctrls);

  /*!< Get external control value */

  esp_err_t (*get_ext_ctrl)(struct esp_video *video,
                            struct v4l2_ext_controls *ctrls);

  /*!< Query external control description */

  esp_err_t (*query_ext_ctrl)(struct esp_video *video,
                              struct v4l2_query_ext_ctrl *qctrl);

  /*!< Set format to sensor */

  esp_err_t (*set_sensor_format)(struct esp_video *video,
                                 const esp_cam_sensor_format_t *format);

  /*!< Get format from sensor */

  esp_err_t (*get_sensor_format)(struct esp_video *video,
                                 esp_cam_sensor_format_t *format);

  /*!< Query menu value */

  esp_err_t (*query_menu)(struct esp_video *video,
