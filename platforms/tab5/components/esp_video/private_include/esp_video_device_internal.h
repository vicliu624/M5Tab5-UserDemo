...
typedef struct esp_video_csi_state {
  ...
  cam_ctlr_color_t
      in_color; /*!< MIPI-CSI input(from camera sensor) data color format */
  cam_ctlr_color_t
      out_color; /*!< MIPI-CSI output(based on ISP output) data color format */
  uint8_t out_bpp; /*!< MIPI-CSI output data color format bit per pixel */
  bool line_sync;  /*!< true: line has start and end packet; false. line has no
                      start and end packet */
  bool
      bypass_isp; /*!< true: ISP directly output data from input port with
                     processing. false: ISP output processed data by pipeline */
 ...
 *
 * @param hw_codec true: hardware H.264, false: software H.264(has not
 * supported)
 *
 * @return
 *      - ESP_OK on success
 ...
 *      - NULL, JPEG video device will create JPEG encoder driver handle by
 * itself
 *      - Not null, JPEG video device will use this handle instead of creating
 ...
 *      - Others if failed
 */
esp_err_t esp_video_isp_start_by_csi(const esp_video_csi_state_t *state,
                                     const struct v4l2_format *format);

/**
 * @brief Stop ISP process
 *
 * @param bypass true: bypass ISP and MIPI-CSI output sensor original data,
 * false: ISP process image
 *
 * @return
 *      - ESP_OK on success
 *      - Others if failed
