...
void ui::pop_a_toast(std::string msg, toast_type::Type_t type,
                     uint32_t durationMs) {
  if (_toast_manager_id < 0) {
    _toast_manager_id = GetMooncake().extensionManager()->createAbility(
        std::make_unique<ToastManager>());
    if (_toast_manager_id < 0) {
  ...
  if (_toast_request_queue.size() >= 5) {
    mclog::tagError(_tag, "toast request queue is full: {}",
                    _toast_request_queue.size());
    return;
  }
