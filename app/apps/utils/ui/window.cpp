...
static smooth_ui_toolkit::Signal<bool>
    _signal_window_opened; // true for opened, false for closed
...
void Window::init(lv_obj_t *parent) {
  ...
  if (config.closeBtn) {
    ...
    _close_btn->align(LV_ALIGN_CENTER,
                      config.kfOpened.x + config.kfOpened.w / 2 - 30,
  ...
  if (config.clickBgClose) {
    ...
    _bg_close_area->setSize(lv_obj_get_width(parent),
...
void Window::update() {
  ...
  // Update state
  if (_anim_x.done() && _anim_y.done() && _anim_w.done() && _anim_h.done() &&
      _anim_opa.done()) {
    if (_state == Opening) {
      _state = Opened;
