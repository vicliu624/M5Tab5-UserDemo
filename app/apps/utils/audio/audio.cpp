...
// 简单线性包络：attack + decay
void generate_tone_with_linear_envelope(std::vector<int16_t> &buffer,
  ...
  for (int i = 0; i < samples; ++i) {
    ...
    if (i < attack_samples) {
      ...
      amplitude = 1.0 - static_cast<double>(i - attack_samples) /
                            (samples - attack_samples); // Linear decay
    }
...
void play_tone(int frequency, double durationSec) {
  ...
  for (int i = 0; i < samples; ++i) {
    ...
    int16_t value = static_cast<int16_t>(
        amp * sin(2.0 * M_PI * frequency * i / sample_rate));
  ...
  for (int midiNote : midiList) {
    for (int i = 0; i < samples_per_note; ++i) {
      ...
      if (midiNote >= 0) {
        ...
        sample = static_cast<int16_t>(amp *
                                      sin(2.0 * M_PI * freq * i / sample_rate));
      }
...
void play_random_tone(int semitoneShift = 0, double durationSec = 0.15) {
  ...
  std::vector<int> scale = {60, 62, 64, 65,
                            67, 69, 71}; // C大调音阶（C D E F G A B）
...
void play_next_chord_progression(double durationSec) {
  ...
  static std::vector<int> chord_roots = {60, 67, 69, 64,
                                         65, 60, 65, 67}; // 15634145
  // static std::vector<int> chord_roots = { 60, 71, 69, 67, 65, 64, 62, 60 };
  // // 17654321 static std::vector<int> chord_roots = { 65, 67, 64, 69, 62, 67,
  // 60 }; // 4536251 static std::vector<int> chord_roots = {65, 65, 64, 69, 62,
  // 67, 60}; // 4436251
  ...
  int root =
      chord_roots[current_chord_index % chord_roots.size()] + SEMITONE_SHIFT;
