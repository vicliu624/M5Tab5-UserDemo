...
static const std::string _toast_msg_charge_disable =
...
static const std::string _toast_msg_ext_antenna_enable =
    "Switched to external antenna";
static const std::string _toast_msg_ext_antenna_disable =
    "Switched to internal antenna";
...
void PanelSwitches::init() {
  ...
  _btn_charge_en_sw->onClick().connect([&]() {
    ...
    ui::pop_a_toast(GetHAL()->getChargeEnable() ? _toast_msg_charge_enable
                                                : _toast_msg_charge_disable,
                    GetHAL()->getChargeEnable() ? ui::toast_type::rose
                                                : ui::toast_type::gray);
    update_images();
  });
  ...
  _btn_charge_qc_en_sw->onClick().connect([&]() {
    ...
    ui::pop_a_toast(GetHAL()->getChargeQcEnable()
                        ? _toast_msg_charge_qc_enable
                        : _toast_msg_charge_qc_disable,
                    GetHAL()->getChargeQcEnable() ? ui::toast_type::orange
                                                  : ui::toast_type::info);
    update_images();
  });
  ...
  _btn_ext_5v_en_sw->onClick().connect([&]() {
    ...
    ui::pop_a_toast(GetHAL()->getExt5vEnable() ? _toast_msg_ext_5v_enable
                                               : _toast_msg_ext_5v_disable,
                    GetHAL()->getExt5vEnable() ? ui::toast_type::error
  ...
  _btn_usba_5v_en_sw->onClick().connect([&]() {
    ...
    ui::pop_a_toast(GetHAL()->getUsb5vEnable() ? _toast_msg_usba_5v_enable
                                               : _toast_msg_usba_5v_disable,
                    GetHAL()->getUsb5vEnable() ? ui::toast_type::error
                                               : ui::toast_type::success);
    update_images();
  });
  ...
  _btn_ext_antenna_en_sw->onClick().connect([&]() {
    ...
    ui::pop_a_toast(GetHAL()->getExtAntennaEnable()
                        ? _toast_msg_ext_antenna_enable
                        : _toast_msg_ext_antenna_disable,
                    ui::toast_type::info);
    update_images();
...
void PanelSwitches::update_images() {
  _img_charge_en_sw->setSrc(GetHAL()->getChargeEnable() ? &sw_chg_on
                                                        : &sw_chg_off);
  _img_charge_qc_en_sw->setSrc(GetHAL()->getChargeQcEnable() ? &sw_qc_on
                                                             : &sw_qc_off);
  _img_ext_5v_en_sw->setSrc(GetHAL()->getExt5vEnable() ? &sw_on : &sw_off);
  _img_usba_5v_en_sw->setSrc(GetHAL()->getUsb5vEnable() ? &sw_on : &sw_off);
  _img_ext_antenna_en_sw->setSrc(GetHAL()->getExtAntennaEnable() ? &sw_rf_h
                                                                 : &sw_rf_l);
}
...
void PanelSwitches::update_detect_images() {
  ...
  //     _last_usb_c_detect = GetHAL()->usbCDetect();
  //     ui::pop_a_toast(_last_usb_c_detect ? _toast_msg_usb_c_connected :
  //     _toast_msg_usb_c_disconnected,
  //                     _last_usb_c_detect ? ui::toast_type::orange :
  //                     ui::toast_type::gray);
  //     play_sfx(_last_usb_c_detect);
  // }
  if (_last_usb_a_detect != GetHAL()->usbADetect()) {
    _last_usb_a_detect = GetHAL()->usbADetect();
    ui::pop_a_toast(_last_usb_a_detect ? _toast_msg_usb_a_connected
                                       : _toast_msg_usb_a_disconnected,
                    _last_usb_a_detect ? ui::toast_type::orange
                                       : ui::toast_type::gray);
  ...
  if (_last_hp_detect != GetHAL()->headPhoneDetect()) {
    _last_hp_detect = GetHAL()->headPhoneDetect();
    ui::pop_a_toast(
        _last_hp_detect ? _toast_msg_hp_connected : _toast_msg_hp_disconnected,
