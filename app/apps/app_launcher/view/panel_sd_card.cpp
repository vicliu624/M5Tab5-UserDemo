...
static const ui::Window::KeyFrame_t _kf_sd_card_scan_close = {-46, 300, 75, 75,
                                                              0};
static const ui::Window::KeyFrame_t _kf_sd_card_scan_open = {-40, 43, 566, 411,
                                                             255};
  ...
  void onUpdate() override {
    ...
    if (GetHAL()->isSdCardMounted() && !_is_scanned) {
      ...
      for (size_t i = 0; i < file_entries.size(); i++) {
        _label_file_entries.push_back(
            std::make_unique<Label>(_panel_file_entries->get()));
        ...
        _label_file_entries.back()->setTextFont(&lv_font_montserrat_24);
        _label_file_entries.back()->setTextColor(
            lv_color_hex(file_entries[i].isDir ? 0xFDBE1A : 0x43D2FF));
