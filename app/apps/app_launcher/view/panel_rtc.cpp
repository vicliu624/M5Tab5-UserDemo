...
static const ui::Window::KeyFrame_t _kf_rtc_setting_close = {436, -219, 75, 75,
                                                             0};
static const ui::Window::KeyFrame_t _kf_rtc_setting_open = {214, 85, 800, 480,
                                                            255};
  ...
  void onUpdate() override {
    if (_state == Opened) {
      if (!_calendar) {
        ...
        _calendar->setTodayDate(local_time->tm_year + 1900,
                                local_time->tm_mon + 1, local_time->tm_mday);
        _calendar->setShowedDate(local_time->tm_year + 1900,
                                 local_time->tm_mon + 1);
        _calendar->onValueChanged().connect([&](lv_calendar_date_t date) {
          _calendar->setTodayDate(date.year, date.month, date.day);
        });
        ...
        _btn_apply->onClick().connect([&]() {
          ...
          ui::pop_a_toast(
              fmt::format("Set RTC time to {}/{}/{} {}:{:02d}:{:02d}",
                          target_time.tm_year + 1900, target_time.tm_mon + 1,
                          target_time.tm_mday, target_time.tm_hour,
...
void PanelRtc::update(bool isStacked) {
  ...
  _label_time->setText(fmt::format("{}:{:02d}:{:02d}", localTime->tm_hour,
                                   localTime->tm_min, localTime->tm_sec));
  _label_date->setText(fmt::format("{}/{}/{}", localTime->tm_year + 1900,
