...
static const ui::Window::KeyFrame_t _kf_ext_port_open = {-438, 68, 273, 380,
                                                         255};
...
public:
  ...
  std::unique_ptr<Container> &get() { return _panel; }

  uint8_t getIoPin() { return _io_pin; }
    ...
    if (_state == Opened) {
      if (_io_panels.empty()) {
        ...
        for (int i = 0; i < _io_pins.size(); i++) {
          ...
          _io_panels.back()->get()->align(LV_ALIGN_TOP_MID, i > 8 ? -124 : 124,
                                          55 + 81 * (i % 9));
...
private:
  ...
  const std::array<uint8_t, 18> _io_pins = {18, 19, 5,  38, 7, 3, 2,  47, 16,
                                            17, 45, 52, 37, 6, 4, 48, 35, 51};
};
