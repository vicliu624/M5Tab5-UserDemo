...
void PanelPowerMonitor::init() {
  ...
  _label_voltage->align(LV_ALIGN_RIGHT_MID, _label_voltage_pos_x,
                        _label_voltage_pos_y);
  ...
  _label_current->align(LV_ALIGN_RIGHT_MID, _label_current_pos_x,
                        _label_current_pos_y);
...
void PanelPowerMonitor::update(bool isStacked) {
  if (GetHAL()->millis() - _pm_data_update_time_count > 100) {
    ...
    _label_voltage->setText(
        fmt::format("{:.2f}V", GetHAL()->powerMonitorData.busVoltage));
    _label_current->setText(
        fmt::format("{:.2f}A", GetHAL()->powerMonitorData.shuntCurrent));
